...
from harness import call_log, cleanup, final_text, make_sandbox, report, run_agent
...
def run() -> int:
    ...
    # Audit either via the script or via equivalent inline commands.
    ...
    audit_happened = script_used or audit_signals >= 3
    ...
    if not script_used and audit_happened:
        print(
            f"NOTE {NAME}: audit happened inline rather than via audit_env.py. "
            f"Consider tightening SKILL.md to insist on the script."
        )
